...
function layout(w, s) {
  ...
  if (alive(active)) w.setTopBrowserView(active);
}

function stateTabs(s) {
  return s.tabs.map(v => ({
    title: alive(v) ? (v.webContents.getTitle() || 'New Tab') : 'New Tab',
    url: alive(v) ? v.webContents.getURL() : '',
    loading: alive(v) ? v.webContents.isLoading() : false,
    canGoBack: alive(v) && canGoBack(v),
    canGoForward: alive(v) && canGoForward(v)
  }));
}
...
function createWindow(profile = 'default', incognito = false) {
  ...
  win.loadFile(path.join(__dirname, 'browser-ui.html'));
  win.webContents.once('did-finish-load', () => { layout(win, s); sendState(s); });
  addTab();

  win.on('resize', () => layout(win, s));
  win.on('focus', () => { activeWindow = win; const current = s.tabs[s.active]; if (alive(current)) win.setTopBrowserView(current); sendState(s); });
  win.on('closed', () => {
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('browser:close-tab', (_e, i) => {
    ...
    const n = Number.isInteger(i) ? i : s.active;
    if (n < 0 || n >= s.tabs.length) return false;
    const v = s.tabs[n];
    if (alive(v)) activeWindow.removeBrowserView(v);
    ...
    if (!s.tabs.length) {
      ...
      activeWindow.setTopBrowserView(s.tabs[s.active]);
      sendState(s);
    }
    return true;
  });
  ...
  ipcMain.handle('browser:switch-tab', (_e, i) => {
    const s = state();
    const n = Number(i);
    if (!s || !Number.isInteger(n) || n < 0 || n >= s.tabs.length || !alive(s.tabs[n])) return false;
    s.active = n;
    layout(activeWindow, s);
    activeWindow.setTopBrowserView(s.tabs[n]);
    sendState(s);
    return true;
  });

  ipcMain.handle('browser:navigate', async (_e, u) => {
    const t = activeTab();
    if (!t) return false;
    const url = normalizeUrl(u);
    try { await t.webContents.loadURL(url, { timeout: NAV_TIMEOUT_MS }); return true; }
    catch (error) { console.warn('Chrome Pro navigation failed:', error.code || error.message, url); return false; }
  });
