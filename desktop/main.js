...
function createWindow(profile = 'default', incognito = false) {
  ...
  const addTab = (u = START_URL) => {
    ...
    s.tabs.push(v); s.active = s.tabs.length - 1; win.addBrowserView(v); win.setTopBrowserView(v);
  ...
  s.addTab = addTab; s.chrome = win.webContents; win.loadFile(path.join(__dirname, 'browser-ui.html')); win.webContents.once('did-finish-load', () => { layout(win, s); sendState(s); }); addTab();
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('browser:tabs', () => { const s = state(); return s ? { active: s.active, profile: s.profile, incognito: s.incognito, tabs: stateTabs(s) } : { active: 0, tabs: [] }; }); ipcMain.handle('browser:history', () => data.history); ipcMain.handle('browser:bookmarks', () => data.bookmarks);
  ipcMain.handle('browser:add-bookmark', (_e, b) => { if (b?.url && !data.bookmarks.some(x => x.url === b.url)) { data.bookmarks.unshift({ title: String(b.title || b.url), url: String(b.url), time: Date.now() }); saveData(); } return data.bookmarks; }); ipcMain.handle('browser:remove-bookmark', (_e, u) => { data.bookmarks = data.bookmarks.filter(x => x.url !== u); saveData(); return data.bookmarks; });
  ipcMain.handle('browser:downloads', () => app.getPath('downloads')); ipcMain.handle('browser:clear-data', async () => { const s = state(); if (s && !s.incognito) { await s.session.clearCache(); await s.session.clearStorageData(); } return true; }); ipcMain.handle('browser:external', async (_e, u) => { if (typeof u === 'string' && /^https?:/i.test(u)) await shell.openExternal(u); return true; });
...
}).catch(e => { console.error('Chrome Pro startup failed:', e); app.quit(); }); app.on('window-all-closed', () => { if (process.platform !== 'darwin') app.quit(); });
